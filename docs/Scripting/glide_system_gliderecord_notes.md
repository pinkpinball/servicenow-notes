# ServiceNow Glide API Notes: GlideSystem (gs) & GlideRecord
...
if (current.state == 7) { // Closed
  gs.log('Incident ' + current.number + ' closed by ' + gs.getUserName(), 'IncidentBR');
}
```
...
- **GlideSystem (gs)**: System info, logs, user info, dates, messages. Never touches tables directly.
